...
#    invited     = 2
    ...
    def check_password(cls, login, password):
        user = DBSession.query(cls).filter(cls.username==login).first() or \
               DBSession.query(cls).filter(cls.email==login).first()
        if user:
            return bcrypt.checkpw(user.password, password)
        else:
            return True
            #return False
